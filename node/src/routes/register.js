...
module.exports = async (req, res, next) => {
    ...
    argon2.hash(password)
        .then(hash => new User({username: xss(username), email: xss(email), firstName: xss(firstName), password: hash, phone: xss(phone), lastName: xss(lastName), lastOnline: Date.now()}).save()

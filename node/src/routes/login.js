...
module.exports = (req, res, next) => {
    ...
    const sendResponse = user => {
        const payload = {id: user._id, email: user.email, level: user.level, firstName: user.firstName, lastName: user.lastName, picture: user.picture, username: user.username};
    ...
    User.findOne(query).populate('picture').populate('endpoint').then(user => {

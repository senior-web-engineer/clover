...
const initSocket = socket => {
    ...
    socket.on('produce', async (data, callback) => {
        ...
        await store.peers.asyncInsert({type: 'producer', socketID: socket.id, userID: socket.decoded_token.id, roomID: data.roomID || 'general', producerID: producer.id, isScreen});

        !producers[socket.id] && (producers[socket.id] = {});
        producers[socket.id][producer.id] = producer;

        socket.to(data.roomID).emit('newProducer', {userID: socket.decoded_token.id, roomID: data.roomID || 'general', socketID: socket.id, producerID: producer.id, isScreen});

        callback({ id: producer.id });
    });

    socket.on('consume', async (data, callback) => {
        const obj = await createConsumer(producers[data.socketID][data.producerID], data.rtpCapabilities, consumerTransports[socket.id]);
    ...
    socket.on('join', async (data, callback) => {
        const user = await User.findOne({_id: socket.decoded_token.id}, {password: 0}).populate('picture');
        socket.to(data.roomID).emit('newPeer', {userID: socket.decoded_token.id, socketID: socket.id, user});
        consumersObjects[data.roomID] = {...(consumersObjects[data.roomID] || {}), [socket.id]: {userID: socket.decoded_token.id, socketID: socket.id, user}}
        ...
        await Meeting.findOneAndUpdate({_id: data.roomID}, { lastEnter: Date.now(), $push: { peers: socket.id }, $addToSet: { users: mongoose.Types.ObjectId(socket.decoded_token.id) } })
            .then(meeting => {
        ...
        callback({producers: peers, consumers: {content: store.consumerUserIDs[data.roomID], timestamp: Date.now()}, peers: consumersObjects[data.roomID]});
    });
    ...
    socket.on('leave', async (data, callback) => {
        ...
        if (store.consumerUserIDs[data.roomID]) store.consumerUserIDs[data.roomID].splice(store.consumerUserIDs[data.roomID].indexOf(socket.id), 1);
        socket.to(data.roomID).emit('consumers', {content: store.consumerUserIDs[data.roomID], timestamp: Date.now()});
